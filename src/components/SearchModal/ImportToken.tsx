...
import { TYPE, CloseIcon, StyledArrowLeft } from "theme";
...
const AddressText = styled(TYPE.main)`
  font-size: 12px;
  ...
  return (
    <Wrapper>
      <PaddedColumn gap="14px" style={{ width: "100%", flex: "1 1" }}>
        <RowBetween>
          {onBack ? <StyledArrowLeft style={{ cursor: "pointer" }} onClick={onBack} /> : <div></div>}
          <TYPE.main fontWeight="bold">Import {tokens.length > 1 ? "Tokens" : "Token"}</TYPE.main>
      ...
      <PaddedColumn gap="md">
        {tokens.map((token) => {
          ...
          return (
            <Card backGround={theme.bg8} key={"import" + token.address} className=".token-warning-container">
              <AutoColumn gap="10px">
                <AutoRow align="center">
                  <CurrencyLogo currency={token} size={"16px"} />
                  &nbsp;
                  <TYPE.main fontWeight={500}> {token.name}</TYPE.main>
                  <TYPE.main ml="8px" mr="8px" fontWeight={500}>
                    ({token.symbol})
                  </TYPE.main>
                </AutoRow>
                {chainId && (
                  <ExternalLink color={theme.brown1} href={getEtherscanLink(chainId, token.address, "address")}>
