...
const StyledTitleText = styled.div<{ active: boolean }>`
  ...
  font-weight: 600;
  color: ${({ theme, active }) => (active ? theme.white : "#614E56")};
`;
...
const RowWrapper = styled(Row)<{ bgColor?: string; active: boolean }>`
  background: ${({ active }) => (active ? "#F29F42" : "#fff4ea")};
        ...
        <RowFixed mt="4px">
          ...
          <StyledMenu ref={node as any}>
            ...
            <PopoverContainer show={open} ref={setPopperElement as any} style={styles.popper} {...attributes.popper}>
              <TYPE.main>{list && listVersionLabel(list.version)}</TYPE.main>
              <SeparatorDark />
              <ExternalLink color={theme.brown1} href={`https://tokenlists.org/token-list?url=${listUrl}`}>
                View list
              </ExternalLink>
    ...
    <Wrapper>
      ...
      {tempList && (
        <PaddedColumn style={{ paddingTop: 0 }}>
          <Card backGround={theme.linear1} padding="12px 20px">
            <RowBetween>
              <RowFixed>
                {tempList.logoURI && <ListLogo logoURI={tempList.logoURI} size="40px" />}
                <AutoColumn gap="4px" style={{ marginLeft: "20px" }}>
                  <TYPE.main color={"theme.white"} fontWeight={600}>
                    {tempList.name}
                  </TYPE.main>
                  <TYPE.brownOpacity color={theme.white} fontSize={"12px"}>
              ...
              {isImported ? (
                <RowFixed>
                  <IconWrapper stroke={theme.green1} size="16px" marginRight={"10px"}>
                    <CheckCircle />
                  </IconWrapper>
                  <TYPE.main color={theme.white}>Loaded</TYPE.main>
                </RowFixed>
              ) : (
                <ButtonPrimary
                  bgcolor={theme.linear2}
                  style={{ fontSize: "14px" }}
                  color={theme.white}
                  padding="6px 8px"
                  width="fit-content"
