...
const ToggleElement = styled.span<{ isActive?: boolean; bgColor?: string }>`
  ...
  height: 24px;
  width: 24px;
  background-color: ${({ isActive }) => (isActive ? "#EC6C45" : "#614E56")};
  :hover {
    opacity: 0.8;
  }
`;
...
const StatusText = styled(TYPE.main)<{ isActive?: boolean }>`
  margin: 0 10px;
  width: 24px;
  color: ${({ theme, isActive }) => (isActive ? "#EC6C45" : theme.text3)};
`;
...
export default function ListToggle({ id, isActive, toggle }: ToggleProps) {
  return (
