...
export class EVM {
  ...
  // Helper function
  assertCertainError(error, expectedErrorMessage) {
    // This complication is so that the actual error will appear in truffle test output
    const message = error.message;
    const matchedIndex = message.search(expectedErrorMessage);
    let matchedString = message;
    if (matchedIndex === 0) {
      matchedString = message.substring(matchedIndex, matchedIndex + expectedErrorMessage.length);
    }
    expect(matchedString).toEqual(expectedErrorMessage);
  }
